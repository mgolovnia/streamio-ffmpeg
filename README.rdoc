...
Suppoted filters: overlay, crop, scale, hflip, vflip, setdar, setsar. 
Example: 
  
Insert a transparent PNG logo in the bottom left corner of the input: 
  
  options = {:overlay => {:movie => "path_to_watermark_file", :x => 10, :y => "main_h-overlay_h-10"}}
  
Scale the input to 2x:
  
  options = {:scale => {:width => "2*iw", :height => "2*ih"}}

Flip the input vertically:
  
  options = {:vflip => true}

For another filters or filter chaining use custom_avfilter: 
 
  options = {:custom_avfilter => "drawbox=10:20:200:60:red@0.5"}

To check filter is installed, use FFMPEG::Transcoder.filter_installed?.
Example:
  
  FFMPEG::Transcoder.filter_installed?("overlay")
